...
def verify_chain():
    """Verify the current blockchain and return True if it is valid, False otherwise"""
...
while waiting_for_input:
    ...
    if user_choice == '1':
        ...
        if len(blockchain) >= 1:
            blockchain[0] = {'previous_hash': '',
                             'index': 0,
                             'transactions': [{'sender': 'Julien', 'recipient': 'Chloe', 'amount': 100}]}
    elif user_choice == 'q':
    ...
    if not verify_chain():
        ...
        break
    print('Balance of {}: {:6.2f}'.format('Julien', get_balance('Julien')))
else:
    print('User left!')
